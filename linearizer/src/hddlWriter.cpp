...
void hddl_output(ostream & dout, ostream & pout, bool internalHDDLOutput, bool usedParsed, bool dontWriteConstantsIntoDomain, bool removeMethodPreconditions){
	...
	bool hasActionCosts = false; // metric_target != dummy_function_type;
	...
	if (!usedParsed){
		for (method m : methods){
			...
			if (m.ordering.size()){
				// printf("DEBUG: Writing out, adding ordering for %s\n", m.name.c_str());
				...
				if (m.ps.size() > 1) {
					printf("Contradictory m.ordering.size and m.ps.size => Bad %s\n" , m.name.c_str());
				}
			}
		...
		for (auto [atname,ms] : parsed_methods) for (parsed_method m : ms){
			...
			if (isTopSortTotalOrder(liftedTopSort,m.tn)){
				printf("Does this also go off, what's going on, %s\n", m.name.c_str());
