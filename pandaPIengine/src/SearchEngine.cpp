...
//    Heuristic *hLMC = new hhLMCount(htn, 0, tnI, lmfFD);
	...
	if (algo == INTERACTIVE){
		...
		bool optimality = args_info.optimality_given;
		...
		solve_with_sat_planner(htn, block_compression, sat_mutexes, pruningMode, effectLessActionsInSeparateLeaf, optimality);
#else
