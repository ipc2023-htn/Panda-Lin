...
void optimally_planning(Model * htn, bool block_compression, bool sat_mutexes, sat_pruning pruningMode, bool effectLessActionsInSeparateLeaf){
	...
	costMode = true;
	currentMaximumCost = 1;
	Depth *maxDepth = new Depth(htn, currentMaximumCost);
	int depthBound = maxDepth->get();
	int depth = 1;
	...
	while (true){
		...
		if (depth > depthBound) {
			currentMaximumCost += 1;
			maxDepth = new Depth(htn, currentMaximumCost);
			depthBound = maxDepth->get();
			depth = 1;
		}
...
void solve_with_sat_planner(Model * htn, bool block_compression, bool sat_mutexes, sat_pruning pruningMode, bool effectLessActionsInSeparateLeaf, bool optimality){
	...
	if (optimality) {
		optimally_planning(htn, block_compression, sat_mutexes, pruningMode, effectLessActionsInSeparateLeaf);
	} else {
